...
- Хранить полный мапинг ***всех*** адресов виртуального адресного пространства в таблице - точно не вариант, большая часть - это неаллоцированные дыры
- Но даже если хранить мапинг только задействованный страниц - это по-прежнему много памяти(т.к. этот мапинг нужно поддерживать для всех процессов)
	- много памяти для хранения в железе
	- если хранить в памяти - это медленно


![](../_resources/Pasted%20image%2020241201003924.png)
- Поэтому есть мапинг наиболее актуальных адресов разных процессов, который хранится в MMU -  ***TLB*** - по сути это кеш полной page table, которая уже лежит в оперативе



- При промахе TLB сам ходит в оперативку и берет нужный физический адрес из page table
- Задача ядра сводится к поддержке маппинга и учете его для каждого процесса, чтобы мониторить его ресурсы
