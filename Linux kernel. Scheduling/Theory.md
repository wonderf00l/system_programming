...
- То есть вызов происходит в процессе из user space --> прокидывание аргументов через стек процесса и регистры в ядро --> исполнение кода ядра в другом процессе(в процесес ядра)
...
- Исполняется
- ***может быть в ожидании какого-то события(данные из сокета и тп)*** - по сути процесс заблокирован - не шедулится планировщиком, не ест процессорное время, занимает только память
- Решедулинг
- Исполнение
- ...
...
- в коде ядра линукса потоки процесса никак не группируются в некие структуры данных аля `threads` и т.п. - потоки процесса объединены только лишь схожими значениями некоторых полей структуры данных `task_struct`(виртуальная память, таблица fd, pid и т.п.)
...
- На уровне выше, различные устройства(HDD, SSD, сетевые карты и т.п.) - по сути, их драйвера - схлопываются в сущность ***устройство ввода-вывода***, с которой работает уже ядро ОС.
	- То есть в ядре нет кода обработки команд драйвера HDD, SSD и т.п. - вместо этого ядро работает с абстракцией(интерфейсом) - на уровне кода это в прямом смысле интерфейс, в C - это набор указателей на функции т.н. называемого устройство ввода-вывода(`read, write, close, open, seek` и т.п.)
...
- Осциллятор генерирует колебания
	...
	- изменение свойств кристалла с известной из физики периодичностью
...
- ОС работает не с конкретными файловыми системами, а с т.н. ***виртуальной файловой системой***, то есть можно реализовать файловую систему, реализовав указанный набор функций
